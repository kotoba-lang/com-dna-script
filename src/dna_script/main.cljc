(ns dna-script.main
  "Kotodama WASM entrypoint for the Dna_script clean-room actor (L4) — Clojure port.
  ...
  facts (`dna_script.<Entity>/<field>`); `*store*` is the in-memory materialization
  used by the contract test and by the WASM runtime before a live engine binds."
  (:require [clojure.string :as str]))

(def ns-prefix "dna_script")
(def tier "L4")
...
(defn emit-facts
  "EAVT facts for one record: {\"dna_script.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "dna_script-compat" :tier tier :entities entities} 200])
...
(defn start! [] :dna_script-compat/ready)
